...
$date_day=$_POST['date_daily_dd'];

$date_search1=$date_year."-".$date_month."-".$date_day;
?> 

<script>
function myFunction() {
    var myWindow = window.open("reports-daily-pdf.php?date_year=<?php echo $date_year; ?>&&date_month=<?php echo $date_month; ?>&&date_day=<?php echo $date_day; ?>", "", "width=950, height=800");
}
</script>
...
<div align="right"><button class="btn btn-success" onclick="myFunction()">Print</button></div>
        ...
        <center>
            <h3><strong>Payment Records for <?php $reports_date=strtotime($date_search1); ?><?php echo "".date("F d, Y", $reports_date)."" ?></strong></h3>
        </center><br>
...
$sql=mysql_query("SELECT users.*,payment.* FROM payment 
    ...
    WHERE payment.date_time LIKE '$date_search1%' ORDER BY payment.paymentID DESC");
