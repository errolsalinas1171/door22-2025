...
$pdf->Ln();
$pdf->Cell(0,6,'Client Records',0,1,'C');
$pdf->Ln();
...
$amount='0';
$sql=mysql_query("SELECT users.first_name,users.last_name,users.username,payment.* FROM payment INNER JOIN users ON payment.userID=users.userID WHERE payment.userID='$userID' ORDER BY payment.paymentID DESC");
        while($row=mysql_fetch_array($sql))
        {
            ...
            $amount_display = number_format($row['amount'], 0);
